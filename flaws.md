...
**Last updated**: v0.9.0 hardening sprint (2026-05-20). Items marked ✅ FIXED were resolved in the Phase B / v0.9 hardening batches.
...
| **Safety enforcement depth (structured)** | Typed invariants, Z3 completeness, fail-safe DENY default on solver failure, adversarial test suite; guard_pipeline non-numeric state bypass FIXED (v0.9) | ✅ | 🟡 | 🟡 | 🟡 | 🟡 | 🔵 | Pramanix leads for numeric/financial/RBAC enforcement. **FIXED (v0.9)**: all 8 guard_pipeline safety checks now raise `SemanticPolicyViolation` with `log.warning` on non-numeric state — no longer a silent bypass. Remaining gap: fast_path fail-open on malformed Decimal inputs (§4.13) and no input sanitisation at `Guard.verify()` boundary | §4.13; commit e8a9c85 | High |
| **NLP safety coverage** | Beta PIIDetector, ToxicityScorer, SemanticSimilarityGuard; detoxify + sentence-transformers optional; model-load failures now observable (v0.9) | 🟡 | 🟡 | 🔵 | ✅ | ✅ | 🟡 | NeMo and GrAI are stronger for broad text moderation, PII redaction, toxicity, jailbreak detection, topic filtering, and general LLM-output safety. **FIXED (v0.9)**: `_try_detoxify_scorer` and `_try_sentence_transformer` now emit `log.warning` on load failure and set `pramanix_nlp_model_available` gauge to 0 — operators can alert before the first unsafe request. NLP validators remain beta-grade; full GrAI/NeMo parity not reached | PROOF_DOSSIER.md: "full Guardrails AI parity not reached"; commit e8a9c85 §5 items 26–27 | High |
...
| **Correctness** | 3,637+ unit tests (0 failures), ≥98% coverage, Hypothesis property tests, Z3 arithmetic completeness proofs; boundary numeric string coverage added (v0.9) | ✅ | 🟡 | 🟡 | 🟡 | 🟡 | 🟡 | Pramanix leads for core decision correctness. **FIXED (v0.9)**: Hypothesis `assume()` over-exclusions resolved; `_semantic_field_equal` boundary strings (NaN, ±inf, Arabic-Indic numerals, underscore syntax) now CI-pinned (§35). Remaining gap: fast_path silent bypass on malformed Decimal inputs (§4.13) | §4.13; commit 0353789 | High |
...
| **Determinism / Audit focus** | Signed decisions, immutable audit log, Ed25519/RS256/ES256 cryptographic tokens, structured provenance; `DecisionSigner` misconfiguration now hard-fails (v0.9) | ✅ | 🔵 | 🔵 | 🟡 | 🟡 | 🔵 | Pramanix leads in verifiable-decision audit trails. **FIXED (v0.9)**: `DecisionSigner.__init__` now raises `ConfigurationError` immediately when the signing key is absent or too short — silent `None`-token audit records are no longer possible. No competitor offers cryptographically signed per-decision audit tokens | commit e8a9c85 §5 item 25 | Medium |
...
| **Memory tooling** | SecureMemoryStore, IFC (information flow control), beta status | 🟡 | 🟡 | 🟡 | 🔵 | 🔵 | ✅ | LlamaIndex is stronger for retrieval, indexing, chunking, document pipelines, RAG-centric workflows. Pramanix memory components are beta and not a retrieval stack | PROOF_DOSSIER.md: "IFC and memory components are beta" | Medium |
...
| Area | Pramanix Status | Pramanix | LC | LG | NeMo | GrAI | LlIdx | Gap Detail | Evidence | Priority |
|------|----------------|----------|----|----|------|------|-------|------------|----------|----------|
| **Ecosystem breadth** | 9 integrations: FastAPI, LangChain, LlamaIndex, AutoGen, Haystack, OpenAI, Anthropic, Cohere, Gemini; LangGraph adapter added (v0.9) | 🟡 | ✅ | ✅ | 🟡 | 🟡 | ✅ | LangChain, LangGraph, and LlamaIndex have far broader connector ecosystems and community plugins. Some Pramanix integrations remain stub implementations; three translators are beta | THESIS.md; integrations/ | High |
| **Deployment surface** | Docker (dev/prod/test/slim), Kubernetes manifests, HPA, NetworkPolicy, ConfigMap | 🟡 | ✅ | ✅ | ✅ | 🟡 | 🟡 | LangChain/LlamaIndex/LangGraph have more community plugins and surrounding tooling (Helm charts, cloud-provider bundles, managed SaaS options). Pramanix stub integrations limit deployment breadth | Phase 6 (SLSA Level 3); deploy/k8s/ | Medium |
...
| Area | Pramanix Status | Pramanix | LC | LG | NeMo | GrAI | LlIdx | Gap Detail | Evidence | Priority |
|------|----------------|----------|----|----|------|------|-------|------------|----------|----------|
| **API stability** | Public API documented (PUBLIC_API.md), versioned; CHANGELOG maintained; `InMemoryExecutionTokenVerifier` removed from top-level namespace (v0.9) | ✅ | ✅ | ✅ | ✅ | 🟡 | ✅ | **FIXED (v0.9)**: `InMemoryExecutionTokenVerifier` removed from `pramanix.__init__` and `__all__`; no longer exported as a first-class production symbol. Remaining gap: API surface has not been tested against a real deprecation cycle; no SemVer stability guarantee until v1.0.0 GA | commit e8a9c85 §5 item 8 | Medium |
| **Packaging consistency** | pyproject.toml with poetry-core; extras restructured; all `noqa: F401` and `type: ignore` suppressions documented (v0.9) | ✅ | ✅ | ✅ | ✅ | 🟡 | ✅ | **FIXED (v0.9)**: all `noqa: F401` production imports annotated with explicit availability-probe or side-effect intent comments. All `type: ignore[operator]` suppressions in `compiler.py` documented with block comments explaining ExpressionNode DSL design intent. No undocumented suppressions remain | commit e8a9c85 §5 items 16, 20 | Low |
| **Observability / ops** | structlog, Prometheus counters, Grafana-ready metrics, circuit-breaker split-brain counter, NLP model availability gauge, WARNING logs on all metric failures (v0.9) | ✅ | 🟡 | 🟡 | 🟡 | 🟡 | 🔵 | **FIXED (v0.9)**: all major silent-swallow observability gaps closed — `pramanix_circuit_breaker_state_sync_failure_total` counter + `log.error` for all 6 Redis sync paths; `_emit_translator_metric` now `log.warning` on failure; `count()` `log.warning` before `return 0`; `pramanix_nlp_model_available{model}` gauge added. Pramanix now has stronger structured observability than most competitors in this space | commit e8a9c85 §5 items 23, 27, 28, 31 | Low |
| **Reliability** | Circuit breaker, token bucket, rate limiting, Redis-backed distributed state; `@functools.cached_property` lock fix; 200-coroutine linearizability test; all reliability-critical silent swallows resolved (v0.9) | ✅ | 🟡 | 🟡 | 🟡 | 🟡 | 🟡 | **FIXED (v0.9)**: `DecisionSigner` raises `ConfigurationError` at init — no silent unsigned records; `count()` logs WARNING before returning 0; split-brain counter + `log.error` in all 6 circuit-breaker Redis paths; 200-coroutine concurrent-mutation lock linearizability test added. Remaining gap: no hyperscale battle-tested production deployment record yet | commit e8a9c85 §5 items 25, 28, 30, 31 | Medium |
| **Test isolation & reproducibility** | 3,637+ unit tests, 0 failures; all bare `sys.modules` assignments wrapped; `try/finally` on module reload tests; Hypothesis `database` and `assume()` issues resolved (v0.9) | ✅ | 🟡 | 🟡 | 🟡 | 🟡 | 🟡 | **FIXED (v0.9)**: all bare `sys.modules` assignments replaced with `patch.dict`/`monkeypatch.setitem`; `database=None` removed from `test_decision_hash.py`; module reload tests wrapped in `try/finally`; all Hypothesis `assume()` over-exclusions closed. Suite runs at 3,637 passed, 86 skipped, 0 failures | commit e8a9c85 §5 items 4, 18, 19 | Low |
...
| Area | Pramanix Status | Pramanix | LC | LG | NeMo | GrAI | LlIdx | Gap Detail | Evidence | Priority |
|------|----------------|----------|----|----|------|------|-------|------------|----------|----------|
| **Ecosystem maturity** | v0.9.0; SLSA Level 3 CI; SBOM; Sigstore; 3,637+ unit tests | 🟡 | ✅ | ✅ | ✅ | ✅ | ✅ | All competitors have multi-year production track records, wider PyPI adoption, established SLAs, and commercial support. Pramanix is technically rigorous but not yet battle-tested at hyperscale | Phase 6/7 status; PROOF_DOSSIER.md | High |
| **Production confidence of secondary layers** | Core Z3 engine is strong; NLP validators, memory, and some integrations are beta; model-load failures now observable (v0.9) | 🟡 | ✅ | ✅ | ✅ | ✅ | ✅ | GrAI and NeMo are more complete for broad safety pipelines because their content-safety and validator ecosystems are production-grade. Pramanix's secondary layers are explicitly beta. **IMPROVED (v0.9)**: NLP model degradation now surfaces via Prometheus gauge and WARNING logs — operators are no longer blind to model-load failures | PROOF_DOSSIER.md; commit e8a9c85 §5 items 26–27 | High |
...
Items marked **✅ FIXED** were resolved in the v0.9.0 hardening sprint. Open items retain their original severity; fixed items are downgraded to reflect the closed risk.
...
| 2 | Safety enforcement depth — guard_pipeline bypass | 8× silent DEBUG pass vs safe-default DENY | 🔴 Critical | ✅ FIXED (v0.9) | All 8 checks now raise `SemanticPolicyViolation` with `log.warning` — non-numeric state is a hard DENY, not a silent pass |
| 3 | NLP safety coverage | Beta validators vs GrAI/NeMo production-grade moderation | 🟠 High | 🟡 Partial (v0.9) | Model-load failures now observable (WARNING + gauge). Validators still beta-grade; promote to production or document explicit beta boundary in PUBLIC_API.md |
...
| 6 | Observability — circuit-breaker split-brain | 6× silent Redis swallows vs any log | 🟠 High | ✅ FIXED (v0.9) | `pramanix_circuit_breaker_state_sync_failure_total` counter added; all 6 Redis sync paths now call `_log.error()` before returning |
| 7 | Determinism / Audit — unsigned records | Silent `None` key vs hard ConfigurationError | 🟠 High | ✅ FIXED (v0.9) | `DecisionSigner.__init__` raises `ConfigurationError` immediately; silent `None`-token audit records are no longer possible |
...
| 9 | Ecosystem breadth | Stub integrations vs mature connector ecosystems | 🟠 High | 🔴 Open | Promote or remove stub integrations; document their status in PUBLIC_API.md with explicit beta/experimental labels |
| 10 | Reliability — quota fail-open | `count()` returns 0 silently vs logged fallback | 🟠 High | ✅ FIXED (v0.9) | `log.warning` now emitted before `return 0` in `RedisExecutionTokenVerifier.count()` — operators can alert on quota-unreliable states |
| 11 | API stability — test double in public API | InMemoryExecutionTokenVerifier exported vs internal-only | 🟡 Medium | ✅ FIXED (v0.9) | Removed from `pramanix.__init__` and `__all__`; no longer a public production symbol |
...
| 13 | NLP safety — fast_path fail-open | 4× silent `return None` vs WARNING + counter | 🟡 Medium | 🔴 Open | Add `pramanix_fast_path_parse_failure_total` counter; emit `log.warning` with input type when Decimal parse fails in a fast-path rule |
| 14 | Test isolation | Bare `sys.modules` assignments, reload order-dependency | 🟡 Medium | ✅ FIXED (v0.9) | All bare assignments replaced with `patch.dict`/`monkeypatch.setitem`; module reload tests wrapped in `try/finally`; Hypothesis `database` and `assume()` gaps resolved |
...
| 16 | Memory tooling | Beta SecureMemoryStore vs LlamaIndex production RAG | 🟡 Medium | 🔴 Open | Define a public `SecureMemoryStore` interface; write a migration guide from LlamaIndex VectorStore to Pramanix-governed retrieval patterns |
| 17 | Packaging consistency | Undocumented `noqa: F401` and `type: ignore` suppressions | 🟡 Medium | ✅ FIXED (v0.9) | All side-effect imports documented with explicit comments; all `type: ignore[operator]` suppressions in `compiler.py` justified with block comments |
