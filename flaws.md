...
| **Safety enforcement depth (structured)** | Typed invariants, Z3 completeness, fail-safe DENY default on solver failure, adversarial test suite | ✅ | 🟡 | 🟡 | 🟡 | 🟡 | 🔵 | Pramanix leads for numeric/financial/RBAC enforcement. Gap: 8× guard_pipeline DEBUG-level bypass (§4.12), fast_path fail-open (§4.13), no input sanitisation at Guard.verify() boundary | §4.12, §4.13, §5 items 21, 29 | Critical |
| **NLP safety coverage** | Beta PIIDetector, ToxicityScorer, SemanticSimilarityGuard; detoxify + sentence-transformers optional | 🟡 | 🟡 | 🔵 | ✅ | ✅ | 🟡 | NeMo and GrAI are stronger for broad text moderation, PII redaction, toxicity, jailbreak detection, topic filtering, and general LLM-output safety. Pramanix's NLP validators are beta, not full-parity | PROOF_DOSSIER.md: "full Guardrails AI parity not reached"; §4.14 (model-load failures invisible) | High |
| **Real LLM adversarial validation** | Dual-model consensus layer, multi-model voting, injection detection in logic layer | 🟡 | 🟡 | 🟡 | ✅ | 🟡 | 🔵 | Layer 4 dual-model consensus is never exercised in CI with real LLMs; all injection tests use stub translators. NeMo has production-tested rail evaluation pipelines. Pramanix's adversarial robustness against live model outputs is unverified | PROOF_DOSSIER.md: "Layer 4 never exercised in CI"; §4.12 guard_pipeline gaps | High |
...
| **Correctness** | 3,923 tests, 98.26% coverage, Hypothesis property tests, Z3 arithmetic completeness proofs | ✅ | 🟡 | 🟡 | 🟡 | 🟡 | 🟡 | Pramanix leads for core decision correctness. Gap: fast_path silent bypass (§4.13), hypothesis assume() over-exclusion (§4.15) leave correctness blind spots for adversarial numeric inputs | §4.13, §4.15; Phase 12 baseline | High |
...
| **Determinism / Audit focus** | Signed decisions, immutable audit log, Ed25519/RS256/ES256 cryptographic tokens, structured provenance | ✅ | 🔵 | 🔵 | 🟡 | 🟡 | 🔵 | Pramanix leads in verifiable-decision audit trails. Gap: `DecisionSigner.__init__` silent `None` key (§4.10), unsigned records if key misconfigured; no competitor offers cryptographically signed per-decision audit tokens | §4.10 audit/signer.py entry; §5 items 25, 34 | High |
...
| Area | Pramanix Status | Pramanix | LC | LG | NeMo | GrAI | LlIdx | Gap Detail | Evidence | Priority |
|------|----------------|----------|----|----|------|------|-------|------------|----------|----------|
| **Ecosystem breadth** | 9 integrations: FastAPI, LangChain, LlamaIndex, AutoGen, Haystack, OpenAI, Anthropic, Cohere, Gemini | 🟡 | ✅ | ✅ | 🟡 | 🟡 | ✅ | LangChain, LangGraph, and LlamaIndex have far broader connector ecosystems and community plugins. Four Pramanix integrations are stub implementations; three translators are beta | THESIS.md: "four integrations are still stub implementations" | High |
...
| Area | Pramanix Status | Pramanix | LC | LG | NeMo | GrAI | LlIdx | Gap Detail | Evidence | Priority |
|------|----------------|----------|----|----|------|------|-------|------------|----------|----------|
| **API stability** | Public API documented (PUBLIC_API.md), versioned; CHANGELOG maintained | 🟡 | ✅ | ✅ | ✅ | 🟡 | ✅ | InMemoryExecutionTokenVerifier is in the public API but is a test double, not production infrastructure (§4.10 item 8 / §5 item 8). API surface has not been tested against a real deprecation cycle. No SemVer stability guarantee beyond v1.0.0 declaration | PUBLIC_API.md; §5 item 8 | High |
| **Packaging consistency** | pyproject.toml with poetry-core; extras restructured; AGPL-3.0 in metadata | 🟡 | ✅ | ✅ | ✅ | 🟡 | ✅ | `extras` restructure was Phase-12 hardening work. `noqa: F401` present on production imports used for side-effects (§3 / §5 item 16); type: ignore[operator] suppressions in compiler.py are undocumented (§5 item 20) | §3.2, §5 items 16, 20 | Medium |
| **Observability / ops** | structlog, Prometheus counters, Grafana-ready metrics, circuit-breaker state metrics | 🟡 | 🟡 | 🟡 | 🟡 | 🟡 | 🔵 | 6× silent Redis circuit-breaker swallows (§4.10) mean split-brain is invisible to Prometheus dashboards. `_emit_translator_metric` outer swallow means LLM failure counters can silently stop (§4.10). `count()` fail-open returns 0 without a WARNING (§5 item 31). NLP model availability not gauged (§5 item 27) | §4.10, §4.12, §5 items 23, 27, 28, 31 | High |
| **Reliability** | Circuit breaker, token bucket, rate limiting, Redis-backed distributed state; `@functools.cached_property` lock fix applied | 🟡 | 🟡 | 🟡 | 🟡 | 🟡 | 🟡 | `DecisionSigner` silent None key can silently drop audit signatures at scale. execution_token `count()` fail-open (return 0) may cause quota bypass. circuit_breaker split-brain risk during Redis outage has no operator alert. No concurrent-mutation integration test for `_lock` after the cached_property fix (§5 item 30) | §4.9 residual gap; §4.10; §5 items 25, 28, 30, 31 | High |
| **Test isolation & reproducibility** | 3,923 tests, 98.26% coverage; `patch.dict` gaps, module reload order dependency | 🟡 | 🟡 | 🟡 | 🟡 | 🟡 | 🟡 | 6× bare `sys.modules` assignments without `patch.dict` (§1.2, §4.4). Hypothesis `database=None` in test_decision_hash.py prevents failure reproduction (§5 item 18). `importlib.reload` order dependency in test_audit_sink_full_coverage.py (§5 item 19) | §1.2, §4.4, §5 items 18, 19 | Medium |
...
| Area | Pramanix Status | Pramanix | LC | LG | NeMo | GrAI | LlIdx | Gap Detail | Evidence | Priority |
|------|----------------|----------|----|----|------|------|-------|------------|----------|----------|
| **Ecosystem maturity** | v0.5.x; SLSA Level 3 CI; SBOM; Sigstore; full test suite | 🟡 | ✅ | ✅ | ✅ | ✅ | ✅ | All competitors have multi-year production track records, wider PyPI adoption, established SLAs, and commercial support. Pramanix is technically rigorous but not yet battle-tested at hyperscale | Phase 6/7 status; PROOF_DOSSIER.md | High |
...
This table shows the single highest-severity gap per area and the minimum action required to close it.

| # | Area | Pramanix vs Best Competitor | Severity | Minimum Action to Close Gap |
|---|------|-----------------------------|----------|-----------------------------|
| 1 | Enterprise adoption / Licence | AGPL-3.0 vs Apache-2.0 (all competitors) | 🔴 Critical | Re-licence core to Apache-2.0 or introduce a commercial licence; update pyproject.toml, README, LICENCE, PROOF_DOSSIER |
| 2 | Safety enforcement depth — guard_pipeline bypass | 8× silent DEBUG pass vs safe-default DENY | 🔴 Critical | §5 items 21–22: replace `_log.debug + continue` with `_log.warning + raise SemanticPolicyViolation` |
| 3 | NLP safety coverage | Beta validators vs GrAI/NeMo production-grade moderation | 🟠 High | Promote detoxify/sentence-transformers validators from beta; add `pramanix_nlp_model_available` gauge (§5 items 26–27, 34) |
| 4 | Real LLM adversarial validation | Stub CI tests vs NeMo production-tested rails | 🟠 High | Add CI integration tests with real (or containerised) LLM endpoints for consensus and injection detection; remove Layer 4 stub dependency |
| 5 | Orchestration depth | Single-action gate vs LangGraph graph-native workflows | 🟠 High | Define and publish a public AgentOrchestrationAdapter protocol; document Pramanix-as-gate pattern for LangGraph state nodes |
| 6 | Observability — circuit-breaker split-brain | 6× silent Redis swallows vs any log | 🟠 High | §5 item 28: add `pramanix_circuit_breaker_state_sync_failure_total` counter + `_log.error()` for all 6 paths |
| 7 | Determinism / Audit — unsigned records | Silent `None` key vs hard ConfigurationError | 🟠 High | §5 item 25: raise `ConfigurationError` at `DecisionSigner.__init__` when key absent; add test (§5 item 34) |
| 8 | Developer UX / Policy authoring | Z3-knowledge required vs no-code schema in GrAI | 🟠 High | Add policy linter with plain-English error messages; add interactive YAML policy validator to CLI |
| 9 | Ecosystem breadth | 4 stub integrations vs mature connector ecosystems | 🟠 High | Promote or remove 4 stub integrations; document their status in PUBLIC_API.md with explicit beta/experimental labels |
| 10 | Reliability — quota fail-open | `count()` returns 0 silently vs logged fallback | 🟠 High | §5 item 31: add `_log.warning()` in `execution_token.py:905` before `return 0` |
| 11 | API stability — test double in public API | InMemoryExecutionTokenVerifier exported vs internal-only | 🟡 Medium | §5 item 8: move to `pramanix._testing`; deprecate public export with a one-version grace period |
| 12 | Benchmark freshness | v0.8.0 consumer laptop vs current hardware | 🟡 Medium | Re-run all benchmarks on v1.0.0 on server-class hardware (8-core, 32 GB RAM); publish in PROOF_DOSSIER.md |
| 13 | NLP safety — fast_path fail-open | 4× silent `return None` vs WARNING counter | 🟡 Medium | §5 item 35: add `pramanix_fast_path_parse_failure_total` counter; log at WARNING with input type |
| 14 | Test isolation | 6× bare `sys.modules` assignments | 🟡 Medium | §5 item 4: replace with `monkeypatch.setitem` or `patch.dict` across all 6 call sites |
| 15 | Policy correctness assurance | No intent-verification vs formal proof | 🟡 Medium | Add a policy simulation/dry-run mode that shows which intents would be allowed/denied with example data, allowing authors to verify intent before deploying |
| 16 | Memory tooling | Beta SecureMemoryStore vs LlamaIndex production RAG | 🟡 Medium | Define a public `SecureMemoryStore` interface; write a migration guide from LlamaIndex VectorStore to Pramanix-governed retrieval patterns |
| 17 | Packaging consistency | `noqa: F401` on side-effect imports, undocumented `type: ignore` | 🟡 Medium | §5 items 16, 20: document side-effect imports with explicit comments; replace or justify each `type: ignore[operator]` |
| 18 | Formal completeness scope | Only covers encoded policy predicates | 🟡 Medium | Add a policy coverage metric: which fields and predicates are declared vs which appear in real traffic; surface uncovered paths in observability dashboard |
