...
| **GA-16** | DX | Missing cloud translators in stub tests | 🟡 Medium | ✅ Fixed 2026-05-30 | Added `tests/integration/test_aws_kms_provider.py` — 7 LocalStack-backed tests for `AwsKmsKeyProvider`: real PEM fetch, public key derivation, `ResourceNotFoundException` → `RuntimeError`, cache TTL, binary secrets, concurrent thread safety. Complements `test_s3_audit_sink.py` (already existed). Real boto3 HTTP round-trips replace all duck-typed module stubs for the AWS path. |
