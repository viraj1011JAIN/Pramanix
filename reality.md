...
**Real Sharp Edge — `_tree_repr` Polynomial/Modulo Gap** (`transpiler.py:730-762`): `_tree_repr` has no match clause for `_PowOp` or `_ModOp`, both of which are fully supported in `transpile()`. Any invariant using `E(x)**n` (polynomial) or `E(x) % n` (modulo) falls through to `Unknown(_PowOp)` / `Unknown(_ModOp)`. This corrupts the `InvariantMeta.tree_repr` structural fingerprint used for equivalence testing and debugging. Z3 verification correctness is unaffected — the bug is in the repr function only, not the transpiler. But policy diff tooling and any code comparing `tree_repr` values will produce wrong output for polynomial policies.
...
3. **The Transpiler (`transpiler.py`):** Lowers the Python AST to `z3.ExprRef` via explicit structural pattern matching (`match/case` starting at line 382).
   - **Confirmed Bug:** `_tree_repr` (line 730) lacks match clauses for `_PowOp` and `_ModOp`, producing degraded fingerprints for polynomial and modulo constraints in `InvariantMeta.tree_repr`. Z3 solving correctness unaffected.
...
Remaining gaps holding back from 9.0+: AGPL-3.0 license (enterprise killer), NLP-based validators absent (PII redaction, toxicity), logging split-brain, Merkle archive encryption gap, HMAC-only JWT (no RS256/ES256). Revised blended score: **8.5**.
...
| Component | File | Capability |
| :--- | :--- | :--- |
| `JWTIdentityLinker` | `identity/linker.py` | Zero-trust JWT identity boundary. JWT signature verified with HMAC-SHA256 **before** any claims are trusted. Caller-provided state in request body is **ignored** — verified `sub` claim is the only state lookup key. Expiry enforced. `StateLoader` Protocol for pluggable state sources. |
...
- **~~`structlog` Dead Weight~~** — **FALSE.** `structlog` is used in `guard_config.py` (core, loaded at import time) for security-critical infrastructure:
  ...
  - **Newly identified gap:** Most other modules (`worker.py`, `solver.py`, `decision.py`, etc.) still use `logging.getLogger(__name__)` — these log events bypass the structlog secrets-redaction pipeline entirely. Sensitive values in those modules can reach log sinks unredacted.
...
| **High** | `circuit_breaker.py` | **Redis TOCTOU Race:** `RedisDistributedBackend.set_state` (line 775) reads state outside the `pipeline(transaction=True)` block. Under concurrent multi-node writes, `failure_count` increments are lost — the circuit breaker trips later than it should under load. Fix: use `HINCRBY` inside the pipeline, or a Lua script. | ✅ Confirmed |
| **Medium** | `transpiler.py` | **Polynomial/Modulo Fingerprint Bug:** `_tree_repr` (line 730) missing match clauses for `_PowOp` and `_ModOp`. Produces `Unknown(...)` in `InvariantMeta.tree_repr`. Z3 correctness unaffected; debugging, policy diff, and equivalence tools produce wrong output for polynomial/modulo policies. Fix: ~6 lines. | ✅ Confirmed |
...
| **Low** | `guard_config.py` + all modules | **Logging Split-Brain:** `guard_config.py` installs structlog with secrets redaction, but most modules use stdlib `logging.getLogger()`. Log events from `worker.py`, `solver.py`, etc. bypass the redaction pipeline entirely. | Newly identified |
...
| **Low** | `identity/linker.py` | **HMAC-SHA256 JWT — Not RS256/ES256.** The `JWTIdentityLinker` uses HMAC-SHA256 (symmetric). In microservice deployments, every service that needs to verify tokens must share the secret key — symmetric key distribution is a security liability. For enterprise deployments (K8s webhooks, multi-service meshes), RS256 or ES256 (asymmetric) is standard. There is no asymmetric JWT verification path in the current implementation. | Newly identified |
...
3. **The logging pipeline has a split-brain security gap.** `guard_config.py` installs structlog with secrets redaction as the first processor. But `worker.py`, `solver.py`, `decision.py`, and most other modules use `logging.getLogger(__name__)` (stdlib), which bypasses structlog entirely. Sensitive policy values in those modules can reach log sinks unredacted. Fix: route stdlib logging through `structlog.stdlib.ProcessorFormatter` so all log events pass through the redaction pipeline, or convert all module-level loggers to `structlog.get_logger()`.

4. **The Redis circuit breaker race condition is a genuine reliability failure.** Under concurrent multi-node deployments, `failure_count` increments from simultaneous writers overwrite each other (`get` outside pipeline → compute → `hset` inside pipeline). The breaker trips late. The entire purpose of the circuit breaker is to prevent cascading Z3 solver overload — a breaker that under-counts failures under load is exactly the scenario it was built to handle. Fix is ~10 lines using `HINCRBY` inside the pipeline.

5. **The `_tree_repr` polynomial bug is a maintenance trap, not a correctness bug today.** It does not affect Z3 decisions. It will silently corrupt `InvariantMeta.tree_repr` for any policy using `E(x)**n` or `E(x) % n`. Any future tooling built on that field — policy diffs, audit reports, invariant-equivalence tests — will produce wrong output. Fix cost: two match clauses in one function.
...
10. **The JWT identity layer uses the wrong algorithm for enterprise deployments.** `JWTIdentityLinker` uses HMAC-SHA256 (symmetric). Every service that needs to verify tokens must hold a copy of the secret key — symmetric key distribution is a security liability in any microservice or K8s mesh deployment. The Kubernetes webhook (`k8s/webhook.py`) is a prime example: every replica must share the same HMAC key, and key rotation requires coordinated restarts. RS256 or ES256 asymmetric verification would allow public key distribution with private key isolation. This is a planned gap, not an oversight — but it must be documented as a limitation for enterprise deployments.
...
2. **Redis Circuit Breaker TOCTOU Race** (`circuit_breaker.py:RedisDistributedBackend`) — The read-outside-transaction pattern in `set_state` causes `failure_count` under-counting under concurrent writes. A circuit breaker that trips late defeats its purpose. Fix: `HINCRBY` inside the pipeline for the counter field.

3. **`_tree_repr` Polynomial/Modulo Gap** (`transpiler.py:730`) — Add match clauses for `_PowOp` and `_ModOp`. Shipping a formal verification framework with broken structural fingerprinting for polynomial constraints undermines confidence in any tooling built on top of it.

**Pre-1.0.0 Recommended (not blockers):**

- Fix logging split-brain: route all stdlib loggers through structlog's `ProcessorFormatter`
