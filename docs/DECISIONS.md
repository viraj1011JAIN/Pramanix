...
**Pramanix 1.0.0** â€” Major design choices, the context in which they were made, and why the alternatives were rejected.

Each entry records the decision, the alternatives that were considered, and the reasons the current approach was chosen. These are not post-hoc rationalizations â€” they are the engineering constraints that ruled alternatives out.
...
**Why fail-closed:**
- If verification infrastructure fails, the correct behavior is to block, not to allow. The alternative â€” allowing on error â€” means a targeted attack on the verification infrastructure can grant access.
...
- `unsat_core()` on a single solver with multiple tracked assertions does not always return the minimal core â€” Z3's core extraction is heuristic. Using one solver per invariant guarantees that `unsat_core()` returns exactly `{label}`, so violation attribution is complete.
...
**Alternatives considered:**
- `threading.local`: Thread-scoped. In asyncio (FastAPI/Uvicorn), multiple concurrent requests share one OS thread. `threading.local` would allow Task B to see Task A's resolved field values â€” a P0 data-bleed.
...
**Why two models:**
- Two independent models must produce the same intent for the same input. A successful prompt injection attack would need to manipulate both models in the same direction simultaneously â€” a significantly harder attack than manipulating one.
...
- Removing `allowed=True` from the fast path as a possibility makes the invariant simple: Z3 is the only path to `allowed=True`. This invariant can be checked statically (and it is â€” there is no code path in `fast_path.py` that produces an `allowed=True` Decision).
...
- The DSL is a library of composable Python objects. There is no string phase. If a user can construct a `ConstraintExpr`, it is because they have called library functions with typed arguments â€” not because they passed a string that the library compiled.
...
- Decision signing (Ed25519) is for offline verifiability by a third party â€” the audit trail must be verifiable by regulators with only the public key. Execution tokens are a runtime control; they are consumed and discarded. Offline verifiability is not required.
...
- The timestamp is preserved in `SignedDecision.issued_at` outside the HMAC boundary â€” available for display but not part of the integrity guarantee.
...
- This is a policy correctness bug that is invisible at runtime â€” the wrong constraint is compiled and wrong decisions are produced silently. Raising at compilation time surfaces the bug immediately.
...
**Why:**
- The recursive implementation hits Python's default call stack limit at approximately 1,000 decisions (logâ‚‚ of 1,000 â‰ˆ 10 recursion levels, but the full hash tree is built pair-wise and each recursion is one level). For production audit logs with tens of thousands of decisions per batch, the recursion limit causes a `RecursionError`.
