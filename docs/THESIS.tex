...
\fname{reality.md} produced a self-assigned score of 9.0/10 (revised upward from
8.5 after Phase~10 confirmed resolution of the Merkle archive encryption gap,
the injection scorer field-name gap, and the async fail-safe bypass).  The primary
...
\begin{enumerate}
  ...
  \item \textbf{NLP content validation.}  PII detection and toxicity scoring
    are entirely absent.  A pluggable validator interface would allow
    \code{spacy}/\code{presidio} integration without hard coupling.

  \item \textbf{Cross-process replay prevention.}  HMAC tags are scoped to a
    single process lifetime.  A persisted, monotonic nonce registry is needed
