...
\textbf{Test suite state} (Python~3.13.7, pytest~7.4.4): \textbf{3,984 passed},
130 skipped, 0 failed.  Branch coverage: \textbf{98.26\,\%} (98\,\% gate:
passing).
...
\textbf{3,984 passed}, \textbf{130 skipped}, \textbf{0 failed}.  Coverage:
...
\item[\code{NaturalPolicySchema}]
  ...
  Fields include: \code{field} (the transaction attribute being constrained),
  \code{operator} (one of \code{lt / le / eq / ge / gt / ne}), \code{value}
  (the threshold), \code{value\_type} (\code{REAL / INT / BOOL / STRING}),
  and optional \code{name} / \code{description} metadata.  All fields are
  validated at construction time; an invalid schema raises \code{ValueError}
  before any Z3 object is created.

\item[\code{ASTBuilder}]
  A stateless transformer that converts a \code{NaturalPolicySchema} into a
  \code{ConstraintExpr} node using the existing Z3 DSL.  The type bridge
  \code{\_Z3\_TO\_PYTHON} maps each \code{value\_type} token to its Python
  counterpart: \code{REAL}→\code{Decimal}, \code{INT}→\code{int},
  \code{BOOL}→\code{bool}, \code{STRING}→\code{str}.  The builder
  validates that the supplied threshold value is coercible to the declared
  type; a mismatch raises \code{TypeError} with a helpful message before any
  Z3 context is entered.
...
\item[\code{MetaVerifier}]
  Runs two Z3 checks on the compiled constraint before the policy is
  accepted.  First, \emph{satisfiability}: the constraint must be satisfiable
  (a policy that is always \textsc{deny} regardless of input is rejected as
  logically vacuous).  Second, \emph{monotonicity}: tightening the threshold
  by a small $\varepsilon$ must produce a strictly more restrictive constraint
  (catches polarity inversions where ``allow amounts under \$100'' was
  accidentally compiled as ``allow amounts over \$100'').  Behaviour is
  governed by \code{VerificationMode}: \code{STRICT} (raise), \code{WARN}
  (log and continue), or \code{SKIP} (no check).

\item[\code{CompiledPolicy}]
  An immutable dataclass holding the original \code{NaturalPolicySchema}, the
  compiled \code{ConstraintExpr}, optional Z3 satisfiability evidence, and a
  SHA-256 digest of the schema for audit traceability.  It exposes a
  \code{to\_policy\_config()} helper that wraps the constraint in a
  \code{PolicyConfig} ready to be passed directly to \code{Guard}.
...
\begin{enumerate}
\item \code{TestNaturalPolicySchemaValidation} (12 tests) — field-level
  Pydantic validation, operator enumeration, value-type enumeration, and
  reject-on-missing-required-field.
\item \code{TestASTBuilderTypeConversion} (8 tests) — each
  \code{value\_type}→Python type mapping, bad-type rejection, and operator
  round-trip.
\item \code{TestASTBuilderConstraintGeneration} (8 tests) — operator
  semantics (\code{lt}, \code{le}, \code{eq}, \code{ge}, \code{gt},
  \code{ne}) and string field support.
\item \code{TestNaturalPolicyCompilerHappyPath} (7 tests) — end-to-end
  compilation of realistic policies (``amounts under \$10{,}000'', KYC tier,
  PEP boolean, currency equality).
\item \code{TestNaturalPolicyCompilerEdgeCases} (6 tests) — dict input
  coercion, missing \code{name}/\code{description} defaulting, large
  threshold values.
\item \code{TestMetaVerifierModes} (6 tests) — \code{STRICT} raises on
  unsatisfiable policy; \code{WARN} logs and returns
  \code{MetaVerificationResult.passed=False}; \code{SKIP} short-circuits
  without calling Z3.
\item \code{TestCompiledPolicyInterface} (5 tests) — immutability, digest
  stability, \code{to\_policy\_config()} integration, repr string.
\item \code{TestEndToEndPolicyApplication} (5 tests) — compiled policy
  applied to a real \code{Guard} instance; correct allow/deny decisions.
\item \code{TestNaturalPolicyRejections} (4 tests) — invalid operator string,
  type coercion failure, contradictory constraint (always false), polarity
  inversion detection.
\end{enumerate}
...
\textbf{Benchmark environment}: single machine, single process, single thread,
\code{sync} execution mode, Python~3.13.7, Z3~4.12.x, \code{orjson} installed,
\code{prometheus-client} installed.  All figures are wall-clock elapsed time,
not CPU time.  Results are not portable across hardware without re-running
...
3,984 tests (Phase~2)       & \fname{reality.md}            & \cfrsec{3.1} \\
