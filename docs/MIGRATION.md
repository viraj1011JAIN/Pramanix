...
### MM-01 — Migrating from LlamaIndex VectorStoreIndex to SecureMemoryStore

**Severity: Additive** (no breaking change; parallel adoption path)

`pramanix.memory.SecureMemoryStore` provides tenant-scoped, IFC-labelled
key-value fact storage with cryptographically enforced cross-tenant isolation.
It satisfies the four retrieval patterns that LlamaIndex `SimpleKVStore` /
`VectorStoreIndex` callers depend on, while adding trust-label enforcement.

**Pattern 1 — Upsert (repeated writes to same key):**

```python
from pramanix.memory import SecureMemoryStore
...
# latest() returns the most recently written entry for that key
latest = store.latest("acme", "run-1", "user_goal")
assert latest.value == "book hotel"
```

**Pattern 2 — Retrieve all facts (equivalent to LlamaIndex retriever.retrieve()):**

```python
entries = store.retrieve("acme", "run-1")            # all keys
entries = store.retrieve("acme", "run-1", "step_1")  # single key
```

**Pattern 3 — Session isolation (maps to LlamaIndex index_id scoping):**

```python
# (tenant_id, workflow_id) is the session scope — strict partition isolation
store.write("tenant-a", "session-1", "secret", value="A1",
            label=TrustLabel.CONFIDENTIAL, source="agent")
# Cross-tenant reads always return empty list — no data escape possible
assert store.retrieve("tenant-b", "session-1") == []
```

**Pattern 4 — Read-only access with max-label ceiling:**

```python
# Only entries at or below INTERNAL are visible to this caller
visible = store.retrieve("acme", "run-1", max_label=TrustLabel.INTERNAL)
assert all(e.label <= TrustLabel.INTERNAL for e in visible)
```

**Pattern 5 — Provenance lineage (extends LlamaIndex metadata):**

```python
entry = store.write("acme", "run-1", "enriched_data",
                    value={"amount": 500}, label=TrustLabel.INTERNAL,
                    source="enricher_agent",
                    lineage=("extractor_agent", "validator_agent"))
assert store.latest("acme", "run-1", "enriched_data").lineage == entry.lineage
```

**Pattern 6 — Index clear (equivalent to LlamaIndex index.clear() / delete_index):**
...
**Trust-label reference:**

| `TrustLabel` | Meaning |
|---|---|
| `UNTRUSTED` | Raw user/external input; lowest trust |
| `PUBLIC` | Safe to expose to any caller |
| `INTERNAL` | Internal pipeline data |
| `CONFIDENTIAL` | Sensitive; only high-trust callers |
| `RESTRICTED` | Highest trust; PII, keys, secrets |

Partitions enforce a `min_label` floor: writes below the floor raise
`MemoryViolationError` — untrusted data cannot leak into a high-trust partition.
