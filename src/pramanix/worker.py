...
_WORKER_POOL_NOT_READY_COUNTER: Any = None
_WORKER_SHED_TOTAL_COUNTER: Any = None
...
try:
    ...
    def _worker_prometheus_register_all() -> None:
        ...
        global _WORKER_POOL_NOT_READY_COUNTER, _WORKER_SHED_TOTAL_COUNTER
        try:
            ...
            _WORKER_POOL_NOT_READY_COUNTER = _worker_prom_register(
                "pramanix_worker_pool_not_ready_total",
                "Requests rejected because WorkerPool was not running",
            )
            _WORKER_SHED_TOTAL_COUNTER = _worker_prom_register(
                "pramanix_worker_shed_total",
                "Requests shed by adaptive load shedder (pool saturated)",
            )
...
def _unseal_decision(
    ...
    *expected_nonce*.  If the echoed nonce in the envelope (``sealed["_n"]``)
    ...
    Args:
        sealed:         The envelope dict produced by
                        :func:`_worker_solve_sealed`.
        expected_nonce: Per-request nonce generated by the host.  Must match
                        the ``_n`` field in *sealed*.
        seal_key:       HMAC key to use for verification.  When ``None``,
                        falls back to the module-level ``_RESULT_SEAL_KEY``
                        (process-local ephemeral key).  In multi-replica
                        deployments inject the shared key from a secrets
                        manager so all coordinators verify with the same key.

    Returns:
        The inner plain decision dict (ready for
        :meth:`WorkerPool._dict_to_decision`).

    Raises:
        ValueError:  HMAC tag does not match OR nonce mismatch — result was
                     tampered with or is a replay of a previous envelope.
        KeyError:    Envelope is malformed (missing ``_p``, ``_t``, or
                     ``_n`` keys).
    """
    ...
    _key = seal_key if seal_key is not None else _RESULT_SEAL_KEY.bytes
    payload = sealed["_p"].encode()
    expected = _hmac_mod.new(_key, payload, hashlib.sha256).hexdigest()
...
class WorkerPool:
    ...
    latency_threshold_ms: float | None = None
    worker_pct: float | None = None
    seal_key: bytes = field(default_factory=lambda: _RESULT_SEAL_KEY.bytes)
    """HMAC-SHA256 key used to sign (worker side) and verify (coordinator side)
    IPC envelopes in ``async-process`` mode.

    Defaults to the module-level ephemeral key (safe for single-coordinator,
    single-process deployments).  For multi-replica K8s deployments inject the
    shared key from a secrets manager so all coordinator pods verify with the
    same key; pass it via ``GuardConfig(result_seal_key=...)``  which Guard
    forwards here at WorkerPool construction time.
    """
    ...
    def submit_solve(
        ...
        """
        if not self._alive:
            if _WORKER_POOL_NOT_READY_COUNTER is not None:
                with contextlib.suppress(AttributeError, ValueError):
                    _WORKER_POOL_NOT_READY_COUNTER.inc()
            return Decision.error(reason="WorkerPool is not running.")

        # Adaptive load shedding
        if not self._shed_limiter.acquire():
            if _WORKER_SHED_TOTAL_COUNTER is not None:
                with contextlib.suppress(AttributeError, ValueError):
                    _WORKER_SHED_TOTAL_COUNTER.inc()
        ...
        try:
            if self.mode == "async-process":
                ...
                try:
                    result_dict = _unseal_decision(
                        sealed, expected_nonce=_ipc_nonce, seal_key=self.seal_key
                    )
